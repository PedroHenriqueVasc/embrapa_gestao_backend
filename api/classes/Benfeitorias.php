...
class Benfeitorias{
    ...
    public function adicionarBenfeitoria(){
        $connection = Connection::getConnection();            

        $query = "INSERT INTO benfeitorias (nome_benfeitoria, caracteristica, qtd, valor, vida_util, tempo_uso, uso_caprinos, uso_ovinos, id_propriedade, id_usuario, data_cadastro) 
        VALUES (:nome_benfeitoria, :caracteristica, :qtd, :valor, :vida_util, :tempo_uso, :uso_caprinos, :uso_ovinos, :id_propriedade, :id_usuario, :data_cadastro)";

        try{

            $stmt = $connection->prepare($query);
            
            $stmt->bindValue(':nome_benfeitoria', $this->nome);
            ...
            $result = $stmt->execute();
            return array('status' => 'Success', 'value' => 'Benfeitoria cadastrada com sucesso!');
    ...
    public function finalizar(){
        ...
        $query = "SELECT nome_benfeitoria, caracteristica, qtd, valor, vida_util, tempo_uso, uso_caprinos, uso_ovinos  FROM benfeitorias WHERE id_usuario = $info->id";
        ...
        if(!$resultados){
            throw new Exception("Nenhuma benfeitoria encontrada!");
        }
